...
const tables = [
  ...
  ["questions", "questions.json", (row) => ({ id: row.id, content: row.content, description: row.description || "", question_group_id: row.questionGroupId, sort_order: row.sortOrder, is_active: row.isActive, template_type: row.templateType || "basic", template_config: row.templateConfig || {}, created_at: row.createdAt, updated_at: row.updatedAt })],
