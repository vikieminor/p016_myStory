...
const tables = [
  ...
  ["my_books", "my-books.json", (row) => ({ id: row.id, book_type_id: row.bookTypeId, title: row.title, sender: row.sender || "", receiver: row.receiver || "", introduction: row.introduction || "", status: row.status, created_at: row.createdAt, updated_at: row.updatedAt, published_at: row.publishedAt || null })],
