...
function sendHtml(res, value) { const html = String(value); const response = html.includes('class="gift-share-page"') ? html.replace('<meta charset="utf-8">', '<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">').replace(/<h1>[^<]*<\/h1>/, '') : value; res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" }); res.end(response); }
