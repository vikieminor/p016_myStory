...
async function createBookRoute(res, body, user) {
  const result = await createBookRecord(body, user, true);
  if (result.error) return sendJson(res, 400, { error: result.error });
  const { book } = result;
  sendJson(res, 201, await presentBook(book));
}

async function createBookRecord(body, user, allowSelf = false) {
  const type = await get("bookTypes", Number(body.bookTypeId));
  if (!type || !type.isActive) return { error: "사용 가능한 북타입을 선택하세요." };
  const title = String(body.title || "").trim() || "오래보고 자세히 보면 모두 어여쁜 인생입니다";
  const isSelf = allowSelf && (body.isSelf === true || body.isSelf === "true" || body.isSelf === "on");
  const sender = String(body.sender || "").trim();
  const receiver = String(body.receiver || "").trim();
  if (!isSelf && (!sender || !receiver)) return { error: "보내는 사람과 받는 사람을 입력하세요." };
  const introduction = String(body.introduction || "").trim() || DEFAULT_RECIPIENT_MESSAGE;
  const book = await create("books", { ownerId: user.id, bookTypeId: type.id, title, sender, receiver, introduction, isSelf, status: "writing" });
  const answerIds = [];
  try {
...
async function createGiftRoute(req, res, body) {
  ...
  try {
    ...
    const created = await createBookRecord(body, user, false);
...
async function renderGiftSharePage(req, res, token) {
  ...
  const body = pages.map((page, index) => `<div class="gift-share-page-shell">${renderBookOutputPage(page, index + 1)}</div>`).join("");
  sendHtml(res, `<!doctype html><html lang="ko"><meta charset="utf-8"><title>${escapeHtml(book.title)} 선물 미리보기</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=KoPub+Batang&family=Reenie+Beanie&family=Rubik:wght@300;400;500&display=swap" rel="stylesheet"><link rel="stylesheet" href="/app.css"><style>
...
async function bookRoute(res, method, id, action, body, access) {
  ...
  if (!action && method === "PUT") {
    const sender = String(body.sender || "").trim();
    const receiver = String(body.receiver || "").trim();
    if (!book.isSelf && (!sender || !receiver)) return sendJson(res, 400, { error: "보내는 사람과 받는 사람을 입력하세요." });
    return sendJson(res, 200, await presentBook(await update("books", id, { title: String(body.title || "").trim() || "오래보고 자세히 보면 모두 어여쁜 인생입니다", sender, receiver, introduction: String(body.introduction || "").trim() || DEFAULT_RECIPIENT_MESSAGE }), true));
  }
...
async function presentBookTypes() {
  const [all, groups, links, questionLinks, questions] = await Promise.all([list("bookTypes"), list("groups"), list("bookTypeGroups"), list("questionBookTypes"), list("questions")]);
  const source = { groups, links, questionLinks, questions };
  return Promise.all(all.sort((a, b) => Number(a.sortOrder || 1) - Number(b.sortOrder || 1) || a.name.localeCompare(b.name, "ko") || Number(a.id) - Number(b.id)).map((type) => presentBookType(type, false, source)));
}
async function presentBookType(type, detail = false, source = null) { const groups = source ? source.groups : await list("groups"); const links = source ? source.links : await list("bookTypeGroups"); const questionLinks = source ? source.questionLinks : await list("questionBookTypes"); const questions = source ? source.questions : await list("questions"); const selected = links.filter((l) => l.bookTypeId === type.id).map((l) => groups.find((g) => g.id === l.questionGroupId)).filter(Boolean).sort(sorter); const result = { ...type, questionGroupIds: selected.map((g) => g.id), questionGroups: selected, questionCount: questions.filter((q) => q.isActive && questionLinks.some((link) => link.questionId === q.id && link.bookTypeId === type.id)).length }; return detail ? { ...result, questions: questions.filter((q) => q.isActive && questionLinks.some((link) => link.questionId === q.id && link.bookTypeId === type.id)).sort(sorter) } : result; }
...
function buildBookOutputPages(details) {
  ...
  if (details.sender || details.receiver || details.isSelf) pages.push({ type: "recipient", sender: details.isSelf ? "" : details.sender, receiver: details.isSelf ? "" : details.receiver, introduction: details.introduction, isSelf: details.isSelf === true });
...
function renderBookOutputPage(page, number, preview = page.preview !== false) {
  ...
  if (page.type === "recipient") { const message = String(page.introduction || "").trim() ? String(page.introduction) : DEFAULT_RECIPIENT_MESSAGE; const people = page.isSelf ? "" : `<div class="book-output-person"><span>보내는 이</span><div class="book-output-person-name"><p>${escapeHtml(page.sender || "")}</p><i aria-hidden="true"></i></div></div>`; const recipient = page.isSelf ? "" : `<div class="book-output-person"><span>받는 이</span><div class="book-output-person-name"><p>${escapeHtml(page.receiver || "")}</p><i aria-hidden="true"></i></div></div>`; return `<div class="book-output-page book-output-recipient"${size}><p class="book-output-recipient-script">My Story</p><div class="book-output-recipient-rule" aria-hidden="true"></div><div class="book-output-recipient-content">${people}<p class="book-output-recipient-message">${escapeHtml(message)}</p>${recipient}</div>${pageNumber}</div>`; }
...
function sendHtml(res, value) { const html = String(value); const response = html.includes('class="gift-share-page"') ? html.replace('<meta charset="utf-8">', '<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">').replace(/<h1>[^<]*<\/h1>/, '').replace('>선물 확인하기</a>', '>[선물 확인하기]</a>') : value; res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" }); res.end(response); }
