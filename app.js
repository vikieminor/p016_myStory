...
async function connectGiftSession() {
  ...
  state.currentAuth = null;
  state.authKind = "account";
  state.giftSession = null;
...
function bookCard(b) { const design = bookTypeDesign(b); const giftReceived = Boolean(state.currentUserId && b.ownerId && String(b.ownerId) !== String(state.currentUserId)); const status = b.status === "published" ? "출판완료" : `작성중 <em>${b.progress}%</em>`; return `<article class="my-book-card book-card" data-go="book/${b.id}"><span class="my-book-status ${b.status === "published" ? "published" : ""}">${giftReceived ? "선물 | " : ""}${status}</span><div class="my-book-main"><div class="my-book-title"><p class="my-book-script">My Story</p><p class="my-book-name">${escapeHtml(b.title)}</p></div><div class="my-book-from-to"><strong>${escapeHtml(b.sender || "보내는 사람이")}(이)가</strong><img src="/assets/home-category-line.svg" alt="" aria-hidden="true"><strong>${escapeHtml(b.receiver || "받는 사람")}에게</strong></div></div><div class="my-book-illustration"><img src="/assets/${design.image}" alt="${escapeHtml(b.bookTypeName)} 유형 일러스트"></div></article>`; }
function giftBookCard(b) { const status = b.status === "published" ? "출판완료" : `작성중 <em>${b.progress}%</em>`; return `<article class="gift-book-card" data-go="book/${b.id}" style="background-color:${escapeHtml(b.coverColor || "")}"><span class="gift-book-status ${b.status === "published" ? "published" : ""}">선물 | ${status}</span><div class="gift-book-main"><div class="gift-book-title"><p class="gift-book-script">My Story</p><p class="gift-book-name">${escapeHtml(b.title)}</p></div><div class="gift-book-from-to"><strong>${escapeHtml(b.sender || "보내는 사람이")}(이)가</strong><img src="/assets/home-category-line.svg" alt="" aria-hidden="true"><strong>${escapeHtml(b.receiver || "받는 사람")}에게</strong></div></div><div class="gift-book-cover"><img src="${escapeHtml(b.coverImageUrl || "")}" alt="${escapeHtml(b.title)} 표지"></div></article>`; }
...
function renderTemplateAnswer(question) {
  ...
  const image = ["image", "short-answer-image"].includes(type) && imageUrl ? `<div class="book-template-reference${type === "short-answer-image" ? " book-template-reference-short-image" : ""}"><p class="question-template-copy">${escapeHtml(config.imageGuidance || "")}</p><img src="${escapeHtml(imageUrl)}" alt="질문 참고 이미지" loading="lazy"></div>` : "";
  const shortAnswerClass = ["short-answer", "short-answer-image"].includes(type) ? ` book-template-short-answer-content${type === "short-answer-image" ? " book-template-short-image-answer-content" : ""}` : "";
  ...
  return image ? `<div class="book-template-answer-content${type === "image" ? " book-template-image-answer-content" : type === "short-answer-image" ? " book-template-short-image-answer-content" : ""}"><textarea id="answerInput" class="book-answer-input" placeholder="나를 기록하는 시간입니다.">${escapeHtml(question?.answer || "")}</textarea>${image}</div>` : `<textarea id="answerInput" class="book-answer-input" placeholder="나를 기록하는 시간입니다.">${escapeHtml(question?.answer || "")}</textarea>`;
}
...
async function admin(section, renderId = state.renderId) {
  ...
  if (section === "gifts") normalizeAdminGiftListActions();
}
...
function normalizeAdminPageStructure() {
  ...
  if (existingHeader) {
    ...
    const questionActions = existingHeader.querySelector(":scope > .admin-question-header-actions");
    const headingRow = document.createElement("div");
    headingRow.className = "admin-page-heading-row";
    headingRow.append(title);
    if (questionActions) {
      questionActions.querySelectorAll(".button.primary").forEach((button) => { button.classList.remove("primary"); button.classList.add("books-create-button"); });
      headingRow.append(questionActions);
    } else {
...
async function adminQuestions() {
  const [data, groups] = await Promise.all([api("/api/questions"), api("/api/question-groups")]);
  const typeCode = (name) => ({ Parents: "P", "Single Parent": "SP", Single: "S", Couple: "C" }[name] || name.slice(0, 1).toUpperCase());
  const bookTypes = (q) => q.bookTypes?.length === 4 ? "All" : (q.bookTypes || []).map((type) => typeCode(type.name)).join(", ") || "-";
  const typeNames = (q) => (q.bookTypes || []).map((type) => type.name);
  const filter = state.adminTypeFilter || "all";
  const visible = filter === "all" ? data.items : data.items.filter((q) => typeNames(q).includes(filter));
  const rows = visible.map((q) => `<tr><td>${q.sortOrder}</td><td title="${escapeHtml(q.content)}">${escapeHtml(shortenText(q.content, 20))}</td><td title="${escapeHtml(q.description)}">${escapeHtml(shortenText(q.description, 10))}</td><td>${escapeHtml(q.questionGroupName)}</td><td>${escapeHtml(bookTypes(q))}</td><td><button class="admin-icon-button" data-toggle-question="${q.id}" aria-label="${q.isActive ? "사용 중지" : "사용"}"><img src="/assets/${q.isActive ? "icn_use_active.svg" : "icn_use_inactive.svg"}" alt=""></button></td><td><div class="admin-icon-actions"><button class="admin-icon-button" data-edit-question="${q.id}" aria-label="질문 수정"><img src="/assets/icn_edit.svg" alt=""></button><button class="admin-icon-button" data-delete-question="${q.id}" aria-label="질문 삭제"><img src="/assets/icn_trash.svg" alt=""></button></div></td></tr>`).join("");
  return `<div class="inline" style="justify-content:space-between"><div><div class="eyebrow">ADMIN</div><h1 class="admin-title">질문 관리</h1></div><div class="inline admin-question-header-actions">${adminTypeFilterSummary(data.items, filter, typeNames)}<button class="button primary" data-open-form="question">+ 질문 등록</button></div></div><div class="panel table-wrap"><table class="table admin-questions-table"><thead><tr><th>번호</th><th>질문내용</th><th>질문설명</th><th>질문그룹</th><th>북타입</th><th>사용</th><th>편집</th></tr></thead><tbody>${rows || `<tr><td colspan="7" class="empty">조건에 맞는 질문이 없습니다.</td></tr>`}</tbody></table></div>`;
}
...
function adminTypeFilterSummary(items, filter, getTypeNames = (item) => [item.bookTypeName]) {
  const types = [["all", "전체"], ["Parents", "P"], ["Single", "S"], ["Single Parent", "SP"], ["Couple", "C"]];
  return `<div class="admin-type-filter-summary" role="group" aria-label="북타입 필터">${types.map(([value, label]) => `<button type="button" class="admin-type-filter${filter === value ? " active" : ""}" data-admin-type-filter="${value}">${label}${value === "all" ? "" : `(${items.filter((item) => getTypeNames(item).includes(value)).length})`}</button>`).join("<span aria-hidden=\"true\">/</span>")}</div>`;
}
...
function normalizeAdminGiftListActions() {
  document.querySelectorAll("[data-admin-gift-detail]").forEach((button) => {
    button.className = "button small admin-gift-detail-button";
    button.textContent = "상세보기";
    button.removeAttribute("aria-label");
  });
  document.querySelectorAll(".admin-gifts-table [data-admin-gift-delete]").forEach((button) => button.remove());
}
...
async function hydrateQuestionTemplateEditor(form) { if (!form?.dataset.id || form.dataset.templateHydrated) return; form.dataset.templateHydrated = "loading"; try { const item = await api(`/api/questions/${form.dataset.id}`); const type = form.elements.templateType; if (type) type.value = ["basic", "short-answer", "image", "short-answer-image", "segmented"].includes(item.templateType) ? item.templateType : "basic"; const config = item.templateConfig && typeof item.templateConfig === "object" ? item.templateConfig : {}; const titles = Array.isArray(config.shortAnswerTitles) ? config.shortAnswerTitles : []; let titleInputs = [...form.querySelectorAll('input[name="shortAnswerTitles"]')]; const addTitleButton = form.querySelector("[data-add-short-title]"); while (titleInputs.length < titles.length && addTitleButton) { addTitleButton.click(); titleInputs = [...form.querySelectorAll('input[name="shortAnswerTitles"]')]; } titles.forEach((value, index) => { if (titleInputs[index]) titleInputs[index].value = value; }); form.querySelectorAll('textarea[name="imageGuidance"]').forEach((input) => { input.value = config.imageGuidance || ""; }); form.querySelectorAll('input[name="imageUrl"]').forEach((input) => { input.value = config.imageUrl || ""; }); type?.dispatchEvent(new Event("change")); form.dataset.templateHydrated = "done"; } catch { form.dataset.templateHydrated = "error"; } }
