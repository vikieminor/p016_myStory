...
async function startApp() {
  if (location.hash.includes("access_token=")) {
    try { await ensureAuthClient(); await authClient.auth.getSession(); history.replaceState(null, "", `${location.pathname}${location.search}`); location.hash = "#moments"; } catch (error) { app.innerHTML = `<div class="panel empty">${escapeHtml(error.message)}</div>`; return; }
  }
  render();
}
...
async function login() {
  ...
  app.innerHTML = `<section class="author-login"><div class="eyebrow">MY STORY</div><h1 class="admin-title">로그인</h1><p class="lead">로그인 후 나의 이야기를 이어가세요.</p><div class="author-login-actions"><button class="button primary" data-google-login>Google로 로그인</button></div><div class="author-login-divider"><span>또는 이메일로 로그인</span></div><form class="panel form" data-form="login"><label class="field">이메일<input type="email" name="email" autocomplete="email" required></label><label class="field">비밀번호<input type="password" name="password" autocomplete="current-password" required></label><button class="button primary" type="submit">이메일로 로그인</button></form><p class="muted author-login-note">Supabase Auth 계정으로 로그인하세요.</p><div class="author-login-divider"><span>또는 선물코드로 시작하기</span></div><form class="panel form gift-login-form" data-form="gift-login"><label class="field">선물코드<input type="text" name="code" autocomplete="off" spellcheck="false" required></label><button class="button primary" type="submit">선물코드로 시작하기</button><p class="gift-login-error" data-gift-login-error role="alert" hidden></p></form></section>`;
}
...
function rememberBookQuestion(bookId, questionId) { if (!state.currentUserId || !questionId) return; try { localStorage.setItem(lastBookQuestionKey(state.currentUserId, bookId), String(questionId)); } catch {} }
async function getShortcutBooks(auth) {
  ...
  return Promise.all(books.map(async (book) => {
    try {
      ...
      return { ...book, shortcutPage: lastQuestion ? `question-${lastQuestion.id}` : "" };
    } catch { return { ...book, shortcutPage: "" }; }
  }));
}
...
function renderTopGnb(auth, writingBooks = []) {
  ...
  topbar.innerHTML = `<a class="home-logo" href="#home">북촌꾸러미연구소</a><nav class="home-nav" aria-label="홈 메뉴">${menu}</nav>`; console.debug("[My Story] GNB rendered", { logoutButton: Boolean(topbar.querySelector("[data-logout]")), logoutButtonHtml: topbar.querySelector("[data-logout]")?.outerHTML || null, loggedIn, isAdmin });
...
async function giftCreate() {
  ...
  app.innerHTML = `<section class="create-page gift-create-page gift-send-page"><div class="create-intro gift-send-intro"><p class="create-kicker">선물하기</p><h1>이제 선물하면 됩니다 :-)</h1><p>오래보고 자세히 보면 모두 아름다운 인생입니다</p></div>${giftSteps(4)}<section class="gift-code-card" aria-labelledby="gift-code-title"><h2 id="gift-code-title">선물 코드</h2><div class="gift-code-row"><code class="gift-code-value" tabindex="0">${escapeHtml(result.code)}</code><button class="gift-code-copy" type="button" data-copy-gift-code aria-label="선물 코드 복사"><img src="/assets/icn_copyofcode.svg" alt=""></button></div><p class="gift-code-help">선물코드로 로그인하면 선물받은 책을 바로 시작할 수 있습니다.</p><p class="gift-copy-feedback" data-gift-copy-feedback hidden>복사되었습니다.</p></section></section>`;
}
...
async function admin(section) {
  const data = await api("/api/bootstrap"); const content = section === "dashboard" ? adminDashboard(data) : section === "questions" ? await adminQuestions() : section === "groups" ? await adminGroups() : section === "types" ? await adminTypes() : section === "covers" ? await adminCovers() : section === "reviews" ? await adminReviews() : section === "moments" ? await adminMoments() : section === "moment-authors" ? await adminMomentAuthors() : section === "banners" ? await adminBanners() : section === "gifts" ? await adminGifts() : await adminPublishing();
...
function adminDashboard(d) { return `<div class="eyebrow">ADMIN</div><h1 class="admin-title">대시보드</h1><div class="admin-dashboard-content"><div class="admin-dashboard-stats"><div class="admin-dashboard-stat"><b>${d.stats.questions}</b><span>전체 질문</span></div><div class="admin-dashboard-stat"><b>${d.stats.groups}</b><span>질문그룹</span></div><div class="admin-dashboard-stat"><b>${d.stats.bookTypes}</b><span>북타입</span></div><div class="admin-dashboard-stat"><b>${d.stats.publishedBooks}</b><span>출판 완료</span></div></div><section class="admin-dashboard-notice"><h2>관리 안내</h2><p class="muted">질문그룹과 질문을 정리한 뒤, 포함할 질문그룹을 북타입에 연결하세요. 새로 만드는 책에는 해당 시점의 활성 질문이 자동으로 추가됩니다.</p></section></div>`; }
async function adminQuestions() { const [data, groups] = await Promise.all([api("/api/questions"), api("/api/question-groups")]); const typeCode = (name) => ({ Parents:"P", "Single Parent":"SP", Single:"S", Couple:"C" }[name] || name.slice(0, 1).toUpperCase()); const bookTypes = (q) => q.bookTypes?.length === 4 ? "All" : (q.bookTypes || []).map((type) => typeCode(type.name)).join(", ") || "-"; const shorten = (value, length) => String(value || "").length > length ? `${String(value).slice(0, length)}…` : String(value || ""); return `<div class="inline" style="justify-content:space-between"><div><div class="eyebrow">ADMIN</div><h1 class="admin-title">질문 관리</h1></div><button class="button primary" data-open-form="question">+ 질문 등록</button></div><div class="panel table-wrap"><table class="table admin-questions-table"><thead><tr><th>번호</th><th>질문내용</th><th>질문설명</th><th>질문그룹</th><th>북타입</th><th>사용</th><th>편집</th></tr></thead><tbody>${data.items.map((q) => `<tr><td>${q.sortOrder}</td><td title="${escapeHtml(q.content)}">${escapeHtml(shorten(q.content, 20))}</td><td title="${escapeHtml(q.description)}">${escapeHtml(shorten(q.description, 10))}</td><td>${escapeHtml(q.questionGroupName)}</td><td>${escapeHtml(bookTypes(q))}</td><td><button class="admin-icon-button" data-toggle-question="${q.id}" aria-label="${q.isActive ? "사용 중지" : "사용"}"><img src="/assets/${q.isActive ? "icn_use_active.svg" : "icn_use_inactive.svg"}" alt=""></button></td><td><div class="admin-icon-actions"><button class="admin-icon-button" data-edit-question="${q.id}" aria-label="질문 수정"><img src="/assets/icn_edit.svg" alt=""></button><button class="admin-icon-button" data-delete-question="${q.id}" aria-label="질문 삭제"><img src="/assets/icn_trash.svg" alt=""></button></div></td></tr>`).join("")}</tbody></table></div>`; }
...
async function adminGifts() { const gifts = await api("/api/admin/gifts"); const typeCode = (name) => ({ Parents:"P", "Single Parent":"SP", Couple:"C", Single:"S" }[name] || name || "-"); const formatDate = (value) => value ? new Date(value).toLocaleDateString("ko-KR") : "-"; return `<div><div class="eyebrow">ADMIN</div><h1 class="admin-title">선물하기</h1><div class="panel table-wrap"><table class="table admin-gifts-table"><thead><tr><th>번호</th><th>보낸 사람</th><th>받는 사람</th><th>책 제목</th><th>북타입</th><th>작성률</th><th>선물일</th><th>마지막 접속</th><th>상태</th><th>편집</th></tr></thead><tbody>${gifts.length ? gifts.map((gift) => `<tr><td>${gift.id}</td><td>${escapeHtml(gift.sender || "-")}</td><td>${escapeHtml(gift.receiver || "-")}</td><td>${escapeHtml(gift.title || "-")}</td><td>${escapeHtml(typeCode(gift.bookTypeName))}</td><td>${gift.progress}%</td><td>${formatDate(gift.createdAt)}</td><td>${formatDate(gift.lastAccessedAt)}</td><td><span class="status ${gift.status === "active" ? "done" : ""}">${escapeHtml(gift.status || "-")}</span></td><td><button class="admin-icon-button" type="button" aria-label="선물 편집 준비 중" disabled><img src="/assets/icn_edit.svg" alt=""></button></td></tr>`).join("") : `<tr><td colspan="10" class="empty">아직 생성된 선물이 없습니다.</td></tr>`}</tbody></table></div></div>`; }
...
async function onClick(e) {
  ...
  if ("createConfirm" in el.dataset) { const created = await api("/api/books", { method: "POST", body: { bookTypeId: state.createType, ...state.bookDraft } }); state.createStep = 1; state.createType = null; state.bookDraft = null; toastMsg("책과 질문 목차를 만들었습니다."); location.hash = `#book/${created.id}`; }
  ...
  if (el.dataset.deleteBook) { if (!confirm("이 책과 작성한 모든 답변, 출판물이 영구 삭제됩니다. 계속하시겠습니까?")) return; try { await api(`/api/books/${el.dataset.deleteBook}`, { method:"DELETE" }); toastMsg("책을 삭제했습니다."); render(); } catch (error) { toastMsg(error.message); } }
  ...
  if (el.dataset.confirmBookDelete) { try { await api(`/api/books/${el.dataset.confirmBookDelete}`, { method:"DELETE" }); document.querySelector(".modal")?.remove(); toastMsg("책을 삭제했습니다."); location.hash = "#books"; } catch (error) { toastMsg(error.message); } }
  ...
  if (el.dataset.googleLogin) { const client = await ensureAuthClient(); const { error } = await client.auth.signInWithOAuth({ provider: "google", options: { redirectTo: `${location.origin}/` } }); if (error) toastMsg(error.message); }
  if ("logout" in el.dataset) { console.debug("[My Story] logout click received", { target: el.outerHTML, currentHash: location.hash }); try { const client = await ensureAuthClient(); console.debug("[My Story] calling Supabase signOut"); const result = await client.auth.signOut(); console.debug("[My Story] signOut result", { error: result.error?.message || null, dataHasSession: Boolean(result.data?.session) }); if (result.error) throw result.error; const sessionAfterSignOut = await getAuthSession(); console.debug("[My Story] session after signOut", { sessionExists: Boolean(sessionAfterSignOut) }); state.authorEditingId = null; if (location.hash === "#home" || location.hash === "") { console.debug("[My Story] rendering logged-out UI"); return render(); } console.debug("[My Story] routing to #home for logged-out UI"); location.hash = "#home"; } catch (error) { console.error("[My Story] logout failed", { name: error.name, message: error.message }); toastMsg(`로그아웃에 실패했습니다: ${error.message}`); } return; }
...
async function openModal(kind, item, coverColumn = "") { const groups = ["question", "type"].includes(kind) ? await api("/api/question-groups") : []; const types = ["question", "type"].includes(kind) ? await api("/api/book-types") : []; const nextTypeSortOrder = Math.max(0, ...types.map(type => Number(type.sortOrder) || 0)) + 1; const title = `${item ? "수정" : "등록"}`; let fields = ""; if (kind === "question") fields = `<label class="field">질문 내용<textarea name="content" required>${escapeHtml(item?.content || "")}</textarea></label><label class="field">질문 설명<textarea name="description">${escapeHtml(item?.description || "")}</textarea></label><label class="field">질문 그룹<select name="questionGroupId" required>${groups.map(g=>`<option value="${g.id}" ${item?.questionGroupId===g.id?"selected":""}>${escapeHtml(g.name)}</option>`).join("")}</select></label><span class="field-label">사용 북타입</span><div class="check-grid admin-question-book-types">${types.map(type=>`<label class="check"><input type="checkbox" name="bookTypeIds" value="${type.id}" ${item ? item.bookTypeIds?.includes(type.id) ? "checked" : "" : "checked"}> ${escapeHtml(type.name)}</label>`).join("")}</div><label class="field">질문 순서<input type="number" min="1" name="sortOrder" value="${item?.sortOrder || 1}" required></label><label class="field">사용 여부<select name="isActive"><option value="true" ${item?.isActive!==false?"selected":""}>사용</option><option value="false" ${item?.isActive===false?"selected":""}>미사용</option></select></label>`;
  ...
  if (kind === "moment") fields = `<label class="field">작가 ID<input name="authorId" value="${escapeHtml(item?.authorId || "")}" required ${item ? "readonly" : ""}></label><label class="field">시간<input type="time" name="slotTime" value="${item?.slotTime || "13:00"}" required ${item ? "readonly" : ""}></label><label class="field">날짜<input type="date" name="momentDate" value="${item?.momentDate || new Date().toISOString().slice(0, 10)}" required></label><label class="field">메모<textarea name="body" required>${escapeHtml(item?.body || "")}</textarea></label><label class="field">노출 여부<select name="isVisible"><option value="true" ${item?.isVisible !== false?"selected":""}>노출</option><option value="false" ${item?.isVisible === false?"selected":""}>숨김</option></select></label>`;
